  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-3">
        {staff.length === 0 ? (
          <p className="text-sm text-slate-500 italic">Aucun staff assigné à cet événement.</p>
        ) : (
          staff.map((s) => (
            <div key={s.id} className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl">
              <div>
                <p className="font-bold text-sm text-slate-900">{s.user.name || s.user.email}</p>
                <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-600 mt-1 flex items-center gap-1">
                  {s.staffRole === 'MANAGER' ? <Shield className="w-3 h-3" /> : <Briefcase className="w-3 h-3" />}
                  {roleLabels[s.staffRole]}
                </p>
              </div>
              {canManage && (
                <button type="button" onClick={() => handleRemove(s.user.id)} className="p-2 text-slate-400 hover:text-rose-600 rounded-xl">
      ...
      {canManage && (
        <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={handleAssign} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-bold">
              Assigner
            </button>
          </div>
        </div>
