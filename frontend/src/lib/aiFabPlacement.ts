...
export function resolveAiFabPlacement(input: {
  ...
  if (onRooms && input.canUseRooms) {
    return {
      ...
      title: `Importer une photo dans l’éditeur, puis Lire avec l’IA · ${AI_ROOM_PLAN_TOKEN_COST} jetons`,
      click: 'open',
      modalTitle: 'Plan de salle IA',
      modalDescription: `Posez une photo dans l’éditeur, puis lancez « Lire avec l’IA ». ${aiTokenCostLegend()}.`,
      highlight: 'room',
      embedSimulator: true,
      inviteHref,
      roomsHref,
