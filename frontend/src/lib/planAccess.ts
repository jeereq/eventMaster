...
export function getFeatureLockMessage(
  ...
  const labels: Partial<Record<keyof PlanCapabilities, string>> = {
    ...
    customRsvpFields: 'Les champs de réponse à l’invitation sur-mesure nécessitent un forfait supérieur',
...
export const QUOTA_GUIDES: Record<QuotaKind, PlanLimitGuide> = {
  events: {
    ...
    why: 'Chaque événement consomme invitations, stockage, envois de réponse et éventuellement de la billetterie.',
  ...
  guests: {
    ...
    why: 'Les envois, la réponse à l’invitation et le plan de table sont dimensionnés selon ce quota.',
