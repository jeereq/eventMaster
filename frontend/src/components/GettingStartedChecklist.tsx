...
export default function GettingStartedChecklist({
  ...
  return (
    <section
      className={cn(
        'rounded-[var(--radius-card)] border border-border bg-surface p-5 sm:p-6 shadow-sm',
        className,
      )}
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 id="getting-started-title" className="text-base font-semibold text-foreground tracking-tight">
              Bien démarrer
            </h2>
            <p className="text-sm text-muted mt-0.5">
              {doneCount}/{steps.length} étapes · parcours recommandé pour votre premier événement
            </p>
          </div>
        </div>
        <button
      ...
      {nextStep && !nextStep.disabled && (
        <div className="mt-4 pt-4 border-t border-border">
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-4 py-2.5 rounded-lg bg-primary hover:bg-primary-hover text-white text-sm font-medium transition shadow-sm shadow-primary/20"
          >
