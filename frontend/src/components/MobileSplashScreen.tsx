...
export default function MobileSplashScreen() {
  ...
  return (
    ...
    >
      <div
        className={cn(
          'flex flex-col items-center gap-3 px-6 text-center transition duration-300 ease-out',
      ...
      >
        ...
        {site.platformTagline ? (
          <p className="text-[10px] font-medium text-muted leading-snug max-w-[16rem] line-clamp-2">
            {site.platformTagline}
          </p>
        ) : null}
        <span className="mt-3 w-5 h-5 rounded-full border-2 border-primary/30 border-t-primary animate-spin" />
      </div>
    </div>
  );
}
