...
function PackItemRow({
  ...
  const titleNode = onOpenListing ? (
    ...
    <Link href={item.href} className={`${titleClass} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 rounded-sm`}>
      {item.title}
    </Link>
  );
      ...
      <div className="flex items-center gap-3">
        {onOpenListing ? (
          <button
            ...
            className="w-11 h-11 sm:w-12 sm:h-12 rounded-[var(--radius-button)] overflow-hidden bg-surface-muted shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
          ...
          <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-[var(--radius-button)] overflow-hidden bg-surface-muted shrink-0">
            {item.coverUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={sizedMediaUrl(item.coverUrl, 96)}
        ...
        <div className="flex flex-wrap items-center gap-x-3 pl-14 sm:pl-[3.75rem]">
          {onKeep ? (
            <button
              type="button"
              onClick={() => onKeep(item)}
              className="inline-flex items-center gap-1 min-h-11 px-1 text-[11px] font-semibold text-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 rounded-sm"
            >
          ...
          {alternatives.length > 0 ? (
            <button
              ...
              aria-expanded={open}
              className="inline-flex items-center gap-1 min-h-11 px-1 text-[11px] font-semibold text-primary hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 rounded-sm"
            >
          ...
          {alternatives.map((alt) => (
            <li key={alt.slug} className="flex items-stretch gap-1">
              <button
                ...
                className="min-w-0 flex-1 min-h-11 text-left rounded-[var(--radius-button)] border border-border px-2 py-2 hover:border-primary/40 hover:bg-surface-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
              >
              ...
              {onOpenListing ? (
                <button
                  ...
                  className="shrink-0 inline-flex items-center justify-center min-h-11 min-w-11 rounded-[var(--radius-button)] border border-border text-muted hover:text-foreground hover:border-primary/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
        ...
        return (
          <article key={pack.id} className="bg-surface border border-border rounded-[var(--radius-card)] p-4 flex flex-col gap-5">
            <header className="flex flex-col gap-1.5">
              <h3 className="font-display text-base font-semibold text-foreground tracking-tight">{pack.label}</h3>
              {pack.blurb ? <p className="text-xs text-muted leading-relaxed">{pack.blurb}</p> : null}
              <p className="text-sm font-semibold text-foreground tabular-nums">{formatFc(pack.totalFc)}</p>
              <p className={cn('text-xs tabular-nums', pack.overBudget ? 'text-rose-700 dark:text-rose-300' : 'text-muted')}>
                {pack.overBudget
