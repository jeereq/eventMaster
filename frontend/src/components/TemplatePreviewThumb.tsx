...
export default function TemplatePreviewThumb({
 ...
 return (
 <div
 className={`w-full max-w-[220px] h-[130px] rounded-xl border border-border/80 dark:border-border overflow-hidden flex flex-col justify-between p-3 shadow-sm ${className}`}
 ...
 <div className="text-[10px] text-muted text-center font-semibold px-2">{name}</div>
 )}
 </div>
 </div>
 );
 ...
 return (
 <div
 className={`w-16 h-20 rounded-lg border border-border dark:border-border overflow-hidden flex flex-col justify-center p-1.5 shrink-0 ${className}`}
 style={{ backgroundColor: bgColor }}
 >
 ...
 <div className="text-[5px] text-muted text-center truncate">{name}</div>
 )}
 </div>
 </div>
 );
