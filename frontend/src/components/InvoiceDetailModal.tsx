      ...
      footer={invoice ? (
        <>
          <button
            ...
            className="flex-1 sm:flex-none min-w-0 px-4 py-2.5 bg-primary-solid hover:bg-primary-solid-hover text-primary-foreground text-xs font-bold rounded-xl transition flex items-center justify-center gap-2 disabled:opacity-50"
          >
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex-1 sm:flex-none px-4 py-2.5 border border-border text-muted text-xs font-bold rounded-xl hover:bg-surface-muted transition"
          >
            Fermer
          </button>
        </>
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-start justify-between gap-4">
                ...
                <span className={cn('px-2.5 py-1 rounded-full text-xs font-bold border shrink-0', statusClass(invoice.status))}>
                  {invoice.statusLabel}
                </span>
              </div>

              <div className="bg-primary/10 border border-primary/20 rounded-xl p-4 text-center">
                <span className="text-xs font-bold text-primary uppercase tracking-wider">Montant TTC</span>
                <p className="text-2xl font-black text-primary mt-1">{invoice.amountFormatted}</p>
              </div>
                    ...
                    {invoice.commissions.map((c) => (
                      <li key={c.id} className="flex items-center justify-between text-sm gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted">{c.commissionRatePercent} % · {c.source}</p>
                        </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                {invoice.tenantName && (
                  <div className="col-span-2 bg-surface-muted p-3 rounded-xl border border-border">
                    ...
                    <p className="font-bold text-foreground mt-1">{invoice.tenantName}</p>
                  </div>
                )}
                <div className="bg-surface-muted p-3 rounded-xl border border-border">
                  ...
                  <p className="font-bold text-foreground mt-1">{invoice.planName || invoice.plan}</p>
                  <p className="text-xs text-muted font-mono">{invoice.plan}</p>
                </div>
                <div className="bg-surface-muted p-3 rounded-xl border border-border">
                  <span className="text-xs font-bold text-muted uppercase">Type</span>
                  <p className="font-bold text-foreground mt-1">{invoice.typeLabel}</p>
                </div>
                <div className="col-span-2 bg-surface-muted p-3 rounded-xl border border-border">
                  <span className="text-xs font-bold text-muted uppercase flex items-center gap-1">
              ...
              {invoice.recipientEmails && invoice.recipientEmails.length > 0 && (
                <div className="text-xs text-muted">
                  <span className="font-bold text-muted">Destinataires initiaux : </span>
              ...
              <div className="border-t border-border pt-4 space-y-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    className="flex-1 min-w-0 px-3 py-2.5 border border-border rounded-xl text-sm bg-surface focus:outline-none focus:ring-2 focus:ring-primary/20"
                  />
                  <button
                    type="button"
                    onClick={() => handleSendEmail(shareEmail || undefined)}
                    disabled={sending}
                    className="w-full sm:w-auto px-4 py-2.5 bg-primary-solid hover:bg-primary-solid-hover text-primary-foreground text-xs font-bold rounded-xl transition flex items-center justify-center gap-1.5 disabled:opacity-50 shrink-0"
                  >
