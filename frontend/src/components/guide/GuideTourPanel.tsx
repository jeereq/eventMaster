...
import { Play, RotateCcw, ListOrdered } from 'lucide-react';
...
export default function GuideTourPanel({ guideId }: GuideTourPanelProps) {
  ...
  const steps = getProductTour(guideId);
    ...
    <div className="space-y-6">
      <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-50 to-violet-50 dark:from-indigo-950/40 dark:to-violet-950/30 border border-indigo-100 dark:border-indigo-900/40 space-y-4">
        <div>
          <h2 className="text-lg font-extrabold text-slate-900 dark:text-white">
            Visite guidée — {getGuideLabel(guideId)}
          </h2>
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">
            Parcourez les menus et fonctionnalités essentielles de votre espace. Relancez cette visite à tout moment.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-3">
        <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
          ...
          Aperçu des étapes ({steps.length})
        </h3>
        <ol className="space-y-2">
          {steps.map((step, i) => (
            ...
            >
              <span className="flex-shrink-0 w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold flex items-center justify-center">
                {i + 1}
              </span>
              <div>
