...
export async function countPaidTicketsForBuyer(eventId: string, buyerEmail: string): Promise<number> {
...
export async function fulfillTicketOrder(orderId: string, stripeSession?: {
  ...
  if (!isDonation || wantDonorPass) {
    ...
    const existingBuyer = await prisma.guest.findUnique({
    ...
    if (isDonation && existingBuyer) {
      ...
      const rawPayloads = Array.from({ length: order.quantity }, (_, i) => {
        ...
        const email = reuseBuyerEmail
          ? buyerEmail
          : companionTicketEmail(buyerEmail, i + 1, order.id);
        return {
          eventId: event.id,
      ...
      for (const row of rawPayloads) {
        ...
        if (!clash) guestPayloads.push(row);
      }
    }
  }
  ...
  if (isDonation) {
    ...
    const html = wrapBrandedEmail({
      ...
      innerHtml: `
        <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#334155;">Bonjour <strong>${escapeHtml(order.buyerName)}</strong>,</p>
        <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#334155;">Votre inscription à <strong>${escapeHtml(event.title)}</strong> est confirmée pour <strong>${order.quantity} place${order.quantity > 1 ? 's' : ''}</strong>.</p>
        ${multiPassesHtml || (seatLine ? `<p style="margin:0 0 16px;font-size:14px;color:#475569;white-space:pre-line;">${escapeHtml(seatLine.trim())}</p>` : '')}
