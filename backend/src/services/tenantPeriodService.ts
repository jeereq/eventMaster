...
import { isB2cPlanKey, getPlanLimitsForTenant } from '../config/plansConfig';

export interface TenantPaidPeriod {
  isPaidPlan: boolean;
...
export interface TenantGuestQuotaUsage {
  periodGuests: number;
 ...
 */
export async function resolveTenantPaidPeriod(tenantId: string): Promise<TenantPaidPeriod> {
  ...
  const isB2c = isB2cPlanKey(tenant.plan);
  ...
  const latestPaidInvoice = await prisma.platformInvoice.findFirst({
    where: {
      tenantId,
      status: 'PAID',
 ...
 */
export async function countTenantGuestsForQuota(tenantId: string): Promise<TenantGuestQuotaUsage> {
  ...
  const limits = getPlanLimitsForTenant(tenant?.plan || PlanType.FREE, tenant?.accountKind);
