...
const twilioWhatsapp = process.env.TWILIO_WHATSAPP_NUMBER || '+14155238886'; // Default Twilio Sandbox number
...
if (twilioSid && twilioAuthToken) {
  ...
  console.warn('[Notification Service] Twilio credentials missing in environment variables. SMS and WhatsApp sending will be simulated.');
}
 ...
 */
export async function sendRealSMS(to: string, body: string): Promise<{ success: boolean; simulated: boolean; messageSid?: string }> {
...
/**
 * Send a real WhatsApp message using Twilio or fall back to simulation
 */
export async function sendRealWhatsApp(to: string, body: string): Promise<{ success: boolean; simulated: boolean; messageSid?: string }> {
  // Ensure phone number is in E.164 format (starts with +)
  let formattedTo = to.trim();
  if (!formattedTo.startsWith('+')) {
    if (formattedTo.startsWith('0')) {
  ...
  if (twilioClient && twilioWhatsapp) {
    try {
      ...
      console.log(`[Notification Service] WhatsApp message sent successfully to ${formattedTo}. SID: ${message.sid}`);
      return { success: true, simulated: false, messageSid: message.sid };
    } catch (error: any) {
      console.error(`[Notification Service] Failed to send WhatsApp to ${formattedTo}:`, error);
      return { success: false, simulated: false };
    }
  } else {
    console.log(`[Simulation] Sending WhatsApp to ${formattedTo}:\nBody: ${body}\n`);
    return { success: true, simulated: true };
  }
}
