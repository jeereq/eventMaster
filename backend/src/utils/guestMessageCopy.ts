...
export const GUEST_COPY = {
  afterInvite:
    'Après confirmation, le badge et l’itinéraire sont dans votre espace invité. Le plan de table et le PDF suivent dès qu’une place vous est assignée.',
  afterReminder:
    'Après votre réponse, le badge et l’itinéraire s’ouvrent dans votre espace invité. Le plan de table et le PDF suivent dès qu’une place vous est assignée.',
  afterRsvp:
    'L’itinéraire est déjà dans votre espace invité. Le plan de table et le PDF suivent dès qu’une place vous est assignée.',
  tableAnnouncement:
    'L’itinéraire est déjà dans votre espace invité. Le PDF d’invitation vous sera envoyé ensuite.',
  ticket:
    'Badge QR et itinéraire : dans votre espace invité. Le plan de table et le PDF suivent dès qu’une place vous est assignée.',
  pdfRequiresRsvp:
    'Le plan de table et le PDF d’invitation seront disponibles une fois votre présence confirmée.',
  inviteEmailFooter:
    'Merci de répondre avant la date de l’événement. Après confirmation, le badge et l’itinéraire sont dans votre espace invité ; le plan de table et le PDF suivent dès qu’une place vous est assignée.',
  rsvpEmailFooter:
    'L’itinéraire est déjà dans votre espace invité. Le plan de table et le PDF suivent dès qu’une place vous est assignée.',
};

const STALE_BLOCKS: Array<{ needles: string[]; replacement: keyof typeof GUEST_COPY }> = [
  {
...
export function rewriteStaleGuestMessageCopy(text: string): string {
  if (!text) return text;
  let next = text;
  for (const block of STALE_BLOCKS) {
    const replacement = GUEST_COPY[block.replacement];
