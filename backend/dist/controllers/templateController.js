...
function canManagePlatformTemplates(user) {
    ...
    if (user.role === 'SUPER_ADMIN')
        return true;
    if (user.role === 'COMMERCIAL' && user.id && (0, platformSettingsService_1.hasCommercialPermission)(user.id, 'canManageTemplates')) {
        return true;
    }
    return false;
}
...
async function resolveComposeImageUrls(body, tenantId) {
    const urls = Array.isArray(body.imageUrls)
        ...
        : [];
    const baseImageUrl = typeof body.baseImageUrl === 'string' && body.baseImageUrl.trim().length > 0
        ? body.baseImageUrl.trim()
        : null;
