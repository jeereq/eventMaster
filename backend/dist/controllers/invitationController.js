...
async function sendInvitation(req, res) {
    try {
        ...
        const sentInvitations = await Promise.all(guests.map(async (guest) => {
            ...
            const guidelinesAlreadyInBody = Boolean(guidelinesText && body.includes(guidelinesText.slice(0, Math.min(24, guidelinesText.length))));
            ...
            for (const chan of channelsToSend) {
                ...
                if (chan === 'EMAIL') {
                    ...
                    if (!destEmail) {
                        ...
                        const htmlBody = (0, brandedMessaging_1.wrapBrandedEmail)({
                            ...
                            innerHtml: `
              ${(0, brandedMessaging_1.messageAlreadyGreets)(body) ? '' : `<p style="font-size:16px;font-weight:700;color:#1e1b4b;margin:0 0 15px;">Bonjour ${(0, brandingUtils_1.escapeHtml)(guest.firstName)},</p>`}
              <div style="font-size:15px;line-height:1.7;color:#475569;margin-bottom:28px;white-space:pre-line;">
                ${(0, brandingUtils_1.escapeHtml)(body.replace(rsvpLink, '')).replace(/\n/g, '<br/>')}
              </div>
              ...
              ${guidelinesText && !guidelinesAlreadyInBody
                      ...
                      <p style="margin:0;font-size:14px;color:#475569;white-space:pre-line;line-height:1.6;">${(0, brandingUtils_1.escapeHtml)(guidelinesText)}</p>
                    </div>`
                                : ''}
            `,
            ...
            const prefs = guest.preferences && typeof guest.preferences === 'object'
                ? guest.preferences
                : {};
        ...
        for (const inv of sentInvitations) {
            ...
            const codes = (inv.channelResults || [])
                .filter((r) => !r.success)
                .map((r) => {
                if (r.failureCode === 'noPhone' || r.failureCode === 'noEmail')
                    return r.failureCode;
                const err = String(r.error || '').toLowerCase();
                if (err.includes('phone') || err.includes('whatsapp') || err.includes('numéro'))
                    return 'noPhone';
                if (err.includes('e-mail') || err.includes('email'))
                    return 'noEmail';
                return 'provider';
            });
            const unique = Array.from(new Set(codes));
            if (unique.includes('noPhone'))
                failureReasons.noPhone += 1;
            else if (unique.includes('noEmail'))
                failureReasons.noEmail += 1;
            else
                failureReasons.provider += 1;
        }
        let message;
...
async function updateInvitation(req, res) {
    try {
        ...
        const updatedInvitation = await db_1.prisma.invitation.update({
            where: { id },
            data: {
...
async function deleteInvitation(req, res) {
    try {
        ...
        await db_1.prisma.invitation.delete({
            where: { id },
        });
