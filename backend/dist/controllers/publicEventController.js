...
function serializePublicEvent(event, donationStats) {
    ...
    const donations = donationsConfig && donationsConfig.enabled && allowedByPlatform
        ? {
            ...donationsConfig,
...
async function checkoutPublicEvent(req, res) {
    try {
        ...
        if (isDonation) {
            const donationsAccess = (0, platformSettingsService_1.getDonationsAccess)();
            const platformCheck = (0, donationsAccess_1.resolveDonationsAccess)(event.tenantId, donationsAccess);
