...
exports.initiateOrgBillingFlexPayPayout = initiateOrgBillingFlexPayPayout;
exports.verifyOrgBillingFlexPayPayout = verifyOrgBillingFlexPayPayout;
...
async function initiateOrgBillingFlexPayPayout(req, res) {
    try {
        const auth = await requireBillingOwner(req, res);
        if (!auth)
            return;
        ...
        await (0, adminAuditService_1.auditReq)(req, {
            ...
            targetId: commercialId,
            tenantId: auth.tenantId,
            summary: `Pay Out FlexPay org. ${(0, commercialPayoutService_1.formatBillingPeriodLabel)(period)} — ${result.transfer?.amountFc} FC`,
            metadata: {
                period,
                transferId: result.transfer?.id,
...
async function verifyOrgBillingFlexPayPayout(req, res) {
    try {
        const auth = await requireBillingOwner(req, res);
        if (!auth)
            return;
        const transferId = String(req.params.transferId || '');
        const transfer = await db_1.prisma.commercialPayoutTransfer.findFirst({
