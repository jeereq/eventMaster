...
exports.DEFAULT_INVITATION_ART_STYLE = void 0;
exports.parseInvitationArtStyle = parseInvitationArtStyle;
exports.invitationArtStyleLabel = invitationArtStyleLabel;
exports.invitationArtStyleScaffoldLine = invitationArtStyleScaffoldLine;
exports.invitationArtStyleImageDirective = invitationArtStyleImageDirective;
exports.invitationArtStyleStructureRules = invitationArtStyleStructureRules;
exports.invitationArtStyleCompositionNote = invitationArtStyleCompositionNote;
exports.invitationArtStyleCraftNotes = invitationArtStyleCraftNotes;
exports.invitationArtStyleLightNote = invitationArtStyleLightNote;
exports.invitationArtStyleFaceLockNote = invitationArtStyleFaceLockNote;
exports.DEFAULT_INVITATION_ART_STYLE = 'realiste';
const ALIASES = {
    realiste: 'realiste',
    realistic: 'realiste',
    photoreal: 'realiste',
...
function parseInvitationArtStyle(raw) {
    if (typeof raw !== 'string')
        return exports.DEFAULT_INVITATION_ART_STYLE;
    const key = raw.trim().toLowerCase();
    return ALIASES[key] || exports.DEFAULT_INVITATION_ART_STYLE;
}
function invitationArtStyleLabel(id) {
    if (id === 'dessin-anime')
        return 'dessin animé';
    if (id === 'illustration')
        return 'illustration';
    ...
    if (id === 'stylise-3d')
        return '3D stylisé';
    if (id === 'art-deco')
        return 'Art Déco Gatsby';
    if (id === 'afro-luxe')
        return 'Afro-Chic Royal';
    if (id === 'gravure-vintage')
        return 'Letterpress & Gravure';
    if (id === 'minimaliste-luxe')
        return 'Quiet Luxury';
    if (id === 'cyber-neon')
        return 'Cyber & Néon';
    return 'réaliste';
...
const CRAFT_FINISH = 'CRAFT FINISH: museum-grade 9:16 print still. One coherent light direction. Color-graded (warm African golds, never grey sludge). Sharp where it must be, soft where it recedes. No extra fingers, no melted ornaments, no duplicate faces, no watermark, no UI chrome, no stock-template look.';
const MATERIALS_RDC = 'MATERIALS: tactile cotton or laid paper; anisotropic gold foil; fresh tropical florals (not plastic roses); authentic wax, kasai velvet or raffia only when the brief asks — never generic European ballroom stock.';
/** Clause [Style] pour le scaffold Nano Banana. */
function invitationArtStyleScaffoldLine(id) {
    switch (id) {
        case 'dessin-anime':
...
function invitationArtStyleImageDirective(id) {
    switch (id) {
        case 'dessin-anime':
            return [
...
function invitationArtStyleStructureRules(id) {
...
function invitationArtStyleCompositionNote(id) {
    if (id === 'realiste') {
...
function invitationArtStyleCraftNotes() {
    return `${CRAFT_FINISH} ${MATERIALS_RDC}`;
}
function invitationArtStyleLightNote(id) {
    switch (id) {
        case 'dessin-anime':
...
function invitationArtStyleFaceLockNote(id) {
    if (id === 'realiste') {
