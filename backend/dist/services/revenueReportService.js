...
exports.parseBillingPeriod = parseBillingPeriod;
exports.buildRevenueReport = buildRevenueReport;
exports.buildRevenueReportCsv = buildRevenueReportCsv;
exports.buildRevenueReportPdf = buildRevenueReportPdf;
const pdfkit_1 = __importDefault(require("pdfkit"));
const db_1 = require("../db");
const commercialService_1 = require("./commercialService");
const invoiceService_1 = require("./invoiceService");
function parseBillingPeriod(input) {
    if (input && /^\d{4}-\d{2}$/.test(input))
        return input;
    return (0, commercialService_1.getBillingPeriod)();
}
async function buildRevenueReport(period) {
    ...
    for (const inv of invoices) {
        if (!byPlan[inv.plan])
            byPlan[inv.plan] = { count: 0, amount: 0 };
    ...
    for (const c of commissions) {
        ...
        if (c.paidAt)
            row.paidCommission += c.commissionAmount;
        else
            row.unpaidCommission += c.commissionAmount;
        row.entries.push({
            tenantId: c.tenantId,
    ...
    const commercialCommissions = Array.from(commercialMap.values()).sort((a, b) => b.totalCommission - a.totalCommission);
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        trendMonths.push((0, commercialService_1.getBillingPeriod)(d));
    }
    const trendInvoices = await db_1.prisma.platformInvoice.groupBy({
        by: ['billingPeriod'],
    ...
    const monthlyTrend = trendMonths.map((m) => {
        ...
        return {
            ...
            revenueFormatted: (0, invoiceService_1.formatAmountFc)(row?._sum.amount ?? 0),
    ...
    return {
        ...
        summary: {
            totalRevenue,
            totalRevenueFormatted: (0, invoiceService_1.formatAmountFc)(totalRevenue),
            invoiceCount: invoices.length,
            totalCommissions,
            totalCommissionsFormatted: (0, invoiceService_1.formatAmountFc)(totalCommissions),
            unpaidCommissions,
            unpaidCommissionsFormatted: (0, invoiceService_1.formatAmountFc)(unpaidCommissions),
            paidCommissions,
            paidCommissionsFormatted: (0, invoiceService_1.formatAmountFc)(paidCommissions),
            netRevenue: totalRevenue - totalCommissions,
            netRevenueFormatted: (0, invoiceService_1.formatAmountFc)(totalRevenue - totalCommissions),
        },
        byPlan,
        invoices: invoices.map((inv) => ({
            ...
            plan: inv.plan,
            amount: inv.amount,
            amountFormatted: (0, invoiceService_1.formatAmountFc)(inv.amount),
            type: inv.type,
            status: inv.status,
            sentAt: inv.sentAt,
...
function buildRevenueReportCsv(report) {
    const lines = [
...
function buildRevenueReportPdf(report) {
    return new Promise((resolve, reject) => {
        ...
        if (report.commercialCommissions.length === 0) {
            ...
            for (const c of report.commercialCommissions) {
                doc.text(`${c.name || c.email} — Commission : ${(0, invoiceService_1.formatAmountFc)(c.totalCommission)} (dû ${(0, invoiceService_1.formatAmountFc)(c.unpaidCommission)}) | CA : ${(0, invoiceService_1.formatAmountFc)(c.totalInvoiceAmount)} | ${c.entries.length} org.`);
            }
        }
        doc.moveDown(1);
        ...
        if (report.invoices.length === 0) {
            ...
            for (const inv of report.invoices) {
                doc.text(`${inv.invoiceNumber} | ${inv.tenantName} | ${inv.plan} | ${inv.amountFormatted} | ${inv.type}`);
            }
        }
        doc.end();
    });
