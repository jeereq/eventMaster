...
exports.buildAdminSubscriptionReport = buildAdminSubscriptionReport;
exports.buildAdminSubscriptionCsv = buildAdminSubscriptionCsv;
exports.manageSubscriptionTarget = manageSubscriptionTarget;
...
function resolveAudienceForPlan(planKey) {
    if (plansConfig_1.B2C_PLAN_KEYS.includes(planKey))
        return 'B2C';
    if (plansConfig_1.VENDOR_PLAN_KEYS.includes(planKey))
        return 'VENUE';
    if (plansConfig_1.B2B_PLAN_KEYS.includes(planKey))
        return 'B2B';
    return 'B2C';
}
async function buildAdminSubscriptionReport() {
    const now = new Date();
    ...
    for (const k of plansConfig_1.PLAN_KEYS) {
    ...
    for (const t of tenants) {
        ...
        const def = (0, plansConfig_1.getPlanLimits)(planKey);
    ...
    const paidConversionRatePercent = totalAccounts > 0 ? Math.round((totalPaidAccounts / totalAccounts) * 100) : 0;
    const plansBreakdown = plansConfig_1.PLAN_KEYS.map((k) => {
        const def = (0, plansConfig_1.getPlanLimits)(k);
...
function buildAdminSubscriptionCsv(report) {
    const header = [
        'ID',
        'Type Cible',
        'Nom',
...
async function manageSubscriptionTarget(params) {
    const { targetType, targetId, plan, action, durationDays, customExpiresAt, accountKind } = params;
    const now = new Date();
    if (targetType === 'tenant') {
        const tenant = await db_1.prisma.tenant.findUnique({ where: { id: targetId } });
        if (!tenant)
            throw new Error('Organisation introuvable.');
        let nextPlan = plan || tenant.plan;
        ...
        const updated = await db_1.prisma.tenant.update({
            ...
            data: {
                ...
                accountKind: accountKind ? accountKind : undefined,
            },
            include: {
    ...
    if (targetType === 'user') {
        ...
        if (!tenantId) {
            ...
            let nextPlan = plan || tenant?.plan || 'PERSONAL_50';
