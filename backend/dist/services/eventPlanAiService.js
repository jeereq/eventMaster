...
async function simulateEventPlanAi(userId, body) {
    ...
    const serviceSlugs = Array.isArray(ai.serviceSlugs)
        ? ai.serviceSlugs.filter((value) => typeof value === 'string' && allowed.has(value) && serviceBySlug.has(value))
        : [];
    ...
    const warnings = Array.isArray(ai.warnings)
        ? ai.warnings.filter((value) => typeof value === 'string' && value.trim().length > 0).slice(0, 6)
        : [];
